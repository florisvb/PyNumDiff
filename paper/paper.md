...
The original PyNumDiff publication [@vanBreugel2022] established the core method set and optimization framework. This version substantially revises it: the taxonomy was reorganized and corrected; the interface unified around explicit keyword arguments; the hyperparameter optimizer was substantially improved; test coverage expanded; and four capabilities were added throughout: multidimensional data, irregular sample spacing, missing observations, and circular domains.
...
Four improvements are made in this version: evaluations are cached to avoid redundant work; the loss is robustified via Huber penalty so outliers do not bias parameter selection; the Kalman parameter space is reduced from two independent noise variances to their log-ratio, the only salient factor [@komarov2025]; and categorical and boolean hyperparameters are supported natively, enabling joint optimization over discrete choices such as derivative order alongside continuous parameters.
