...
try:
    ...
    warnings.warn('Could not import pychebfun. Install pychebfun (https://github.com/pychebfun/pychebfun/) to use chebfun derivatives.')
try:
    import pydmd.dmdc
except:
...
try:
    ...
    warnings.warn('Could not import cvxpy. Install cvxpy (http://www.cvxpy.org/install/index.html) to use lineardiff. \
...
def __dmddiff__(X, dt, params, options={}):
    ...
    # DMD reconstruction
    x0 = X[:, 0]
    Xr = dmdc.forward_backward_reconstruct(x0, X.shape[1], 0, overlap=2)
...
def dmddiff(x, dt, params, options={'sliding': True, 'step_size': 10, 'kernel_name': 'gaussian'}):
    ...
    if 'sliding' in options.keys() and options['sliding'] is True:
        ...
        integral_x = utility.integrate_dxdt_hat(x, dt)
        X = utility.hankel_matrix(np.matrix(integral_x), delay_embedding).T
        return __dmd_smooth__(X, dt, params, options={})
...
def spectraldiff(x, dt, params, options={'even_extension': True}): 
    '''
    ...
    dxdt_hat = dxdt_hat[0:original_L]

    # Integrate to get x_hat
    x_hat = utility.integrate_dxdt_hat(dxdt_hat, dt) 
    x0 = utility.estimate_initial_condition(x[0:original_L], x_hat)
    x_hat = x_hat + x0
