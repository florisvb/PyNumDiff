...
method_params_and_bounds = {
    ...
    rbfdiff: ({'sigma': [1e-2, 1e-1, 1],
              ...
              {'sigma': (1e-2, 1e3),
                'lmbd': (1e-3, 0.5)}),
    waveletdiff: ({'wavelet': {'db4', 'db8', 'db12', 'sym8', 'coif1', 'coif2', 'bior4.4'},
                 'mode': {'symmetric', 'reflect', 'antireflect', 'constant'}, # 'zero' and 'periodic' cause jumps, and 'smooth' extrapolates noise
                 'threshold': [1.0, 2, 4]}, # `level` is left to its adaptive default
                {'threshold': (0.1, 10)}),
    ...
    lineardiff: ({'kernel': {'gaussian', 'friedrichs'}, # `stride` barely moves accuracy so now defaults to window_size//5
                   'order': {2, 3}, # 1 fits only exponentials, and 4 tends to overfit the proxy cost
             ...
             'window_size': [41, 81, 161]}, # 2*order unknowns fit per row of X, so windows too small tend to be poorly determined
                  {'gamma': (1e-6, 1e1),
             'window_size': (11, 1000, 'odd')}) # stride locked to 1//5th window size here to save time
}
